...
export default class PageCreater {
    ...
    __extendedPages__;
    __category_keys__;
    __category_page_data__;
    __blogs_to_be_released__;
    constructor(app, options, themeConfig) {
        this.app = app;
    ...
    get categoryPaginationPosts() {
        let paginationData = {};
        this.__category_keys__.forEach((categoryKey) => {
            ...
            categoryValues.forEach((categoryValue) => {
                ...
                const paginationDataOfCategoryValue = Array.from({ length: totalPage }).reduce((prev, current, index) => {
    ...
    // 将 path 中的中文转换成拼音
    _parseChineseInPagePathToPinyin() {
    ...
    _parseCategoryPageOptions(option) {
        const { layout, pageSize, frontmatterKey: categoryKey, } = option;
    ...
    _setBlogsToCategoryPageData() {
        ...
        blogsToBeReleased.forEach((page) => {
            ...
            categoryKeysOfFrontmatter.forEach((key) => {
                ...
                if (isEmptyPlainObject(this.__category_page_data__[key].items)) {
                    this.__category_page_data__[key].items = categoryValues.reduce((total, categoryValue) => {
                    ...
                    categoryValues
                        ...
                        if (!categoryValue)
                            return;
                        const itemKey = formatCategory(categoryValue);
                        const categoryPageDataItem = this.__category_page_data__[key].items[itemKey];
    ...
    // 设置系列
    _setSeries(page) {
        let matches = (page.filePath || '').match(/\/series\/([^/]+)(\S*?)\/([^/.]+)\.md/);
        if (!matches)
            return;
