...
![PixPin_2025-12-15_17-01-20](./08%E4%BB%BB%E5%8A%A1%E4%B8%8E%E8%BF%9B%E7%A8%8B.assets/PixPin_2025-12-15_17-01-20-1765806434046-1.gif)
...
`crond`是Linux操作系统下用来周期性的执行某种任务或等待处理某些事件的一个守护进程，与Windows 操作系统下的计划任务类似。

```shell
语法：
crond [选项] 
    -h         打印这条消息                                                                   
    -i         以不支持inotify的方式运行守护进程                   
    -m <comm>  关闭该功能，或指定用于发送邮件的客户端                   
    -n         在前台运行                                                                   
    -f         在前台运行，与-n的作用相同                  
    -p         允许使用任何crontab任务                    
    -P         从环境中继承PATH变量，而不是使用默认值“/usr/bin:/bin”        
    -c         启用集群支持功能                             
    -s         将日志信息发送到syslog系统中，而不是通过邮件发送                
    -V         打印版本信息后退出程序         
    -x <flag>  打印调试信息
```

`crontab`是定时任务的配置文件，与crond命令有本质上 的区别，crond用于执行某些定时任务，而crontab是定时任务的配置文件。我们可以通过配置文件来实现定时任务的增删改。crontab依赖于crond执行。

**总结一句话就是：crond执行，crontab创建任务。**

首先我们要启动crond的服务。操作如下：

![image-20251216095809945](./08%E4%BB%BB%E5%8A%A1%E4%B8%8E%E8%BF%9B%E7%A8%8B.assets/image-20251216095809945.png)

由于我们创建定时任务的时候使用的是/etc/crontab文件，所以我们只需要在文件中添加新的任务计划即可。

我们想查看目前有哪些定时任务，操作如下：

![image-20251216100154615](./08%E4%BB%BB%E5%8A%A1%E4%B8%8E%E8%BF%9B%E7%A8%8B.assets/image-20251216100154615.png)

目前用户没有任何定时任务。我们接下来创建一个定时任务，我们先学会查看配置文件，并且能够读懂里面的内容

![image-20251216101543153](./08%E4%BB%BB%E5%8A%A1%E4%B8%8E%E8%BF%9B%E7%A8%8B.assets/image-20251216101543153.png)

我们可以看见，每一个*都对应一个选项，第一个是分钟，0-59；第二个是小时，0-23，第三个是天，1-31；第四个是月，1-12；第五个是一周中的星期几，0-7。
...
```shell
0 3 * * 0 /usr/bin/find /var/log -type f -mtime +7 -delete
```
...
crontab -l
# 编辑 crontab 文件(编辑定时任务)
crontab -e
# 删除 crontab 文件(删除定时任务)
crontab -r
# 删除crontab 文件提醒用户（删除定时任务)
crontab -i
```

其实 crontab 定时任务非常简单，只需执行“crontab-e"命令，然后输入想要定时执行的任务即可。不过，当我们执行"crontab-e"命令时，打开的是一个空文件，而且操作方法和Vim 是一致的。

#### 脚本 + crond

比如说我们写一个定时的clean_cache.sh脚本内容如下：

```bash
#!/bin/bash
PATH=/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin

echo "start clean cache" >> /tmp/cron.log
```

注意写的时候要加上PATH，不然容易失败。

```shell
0 2 * * * /bin/bash /opt/scripts/clean_cache.sh
```

将执行结果写入日志

```shell
*/10 * * * * /root/test.sh >> /var/log/test.log 2>&1
```
...
```shell
# /etc/crontab
0 4 * * * root /usr/bin/yum -y update
```

### 一次性定时任务

`at`命令是一次性定时计划任务，at 的守护进程 atd 会以后台模式运行，检查作业队列来运行作业。atd 守护进程会检查系统上的一个特殊目录来获取 at 命令的提交的作业，默认情况下，atd 守护进程每 60 秒检查一次目录，有作业时，会检查作业运行时间，如果时间与当前时间匹配，则运行此作业。
...
at [选项] [时间]
    ...
    -V：显示任务将被执行的时间
    -c：打印任务的内容到标准输出-V显示版本信息
    -t：<时间参数>以时间参数的形式提交要运行的任务
```

首先我们想查看atd服务是否在运行，操作如下：

![image-20251216104547427](./08%E4%BB%BB%E5%8A%A1%E4%B8%8E%E8%BF%9B%E7%A8%8B.assets/image-20251216104547427.png)

服务是在运行的，如果没有运行，我们就需要使用`systemctl start atd`来启动服务。

at有两个权限控制文件，一个是白名单/etc/at.allow，一个是黑名单/etc/at.deny，如果两个文件里面都是空的，那么就是有root用户可以使用。

我们使用at命令之后会进入到交互模式，输入命令，使用`Ctrl+D`进行提交。
...
![image-20251216110425818](./08%E4%BB%BB%E5%8A%A1%E4%B8%8E%E8%BF%9B%E7%A8%8B.assets/image-20251216110425818.png)
...
| `at now + 1 hour`     | 1 小时后         |
| `at 23:30`            | 今天或明天 23:30 |
| `at 10am`             | 上午 10 点       |
| `at 2025-12-20 03:00` | 指定日期时间     |

假如我们俩要在晚上23点执行一个脚本，命令操作如下：
...
`atq`命令用于查看当前用户（或 root）的一次性任务，操作如下：

![image-20251216110639526](./08%E4%BB%BB%E5%8A%A1%E4%B8%8E%E8%BF%9B%E7%A8%8B.assets/image-20251216110639526.png)

`atrm`用于删除任务，操作如下：

![image-20251216110742712](./08%E4%BB%BB%E5%8A%A1%E4%B8%8E%E8%BF%9B%E7%A8%8B.assets/image-20251216110742712.png)
